SELECT
  ...
  visitstarttime*1000000 + hits.time*1000 AS usecTime,
  FullVisitorId,
  VisitId,
  VisitNumber,
...
FROM (TABLE_DATE_RANGE([121095747.ga_sessions_], DATE_ADD(CURRENT_TIMESTAMP(), -29, 'DAY'), DATE_ADD(CURRENT_TIMESTAMP(), -1, 'DAY'))),
  (TABLE_DATE_RANGE([121095747.ga_sessions_intraday_], DATE_ADD(CURRENT_TIMESTAMP(), -30, 'DAY'), CURRENT_TIMESTAMP()))

  WHERE
  hits.eventInfo.eventValue is not null

GROUP EACH BY hits.hitnumber, hitnumber, hitTimestamp, usectime, visitid, visitnumber, appName, eventCategory, eventAction, eventLabel, eventValue, fullVisitorId
